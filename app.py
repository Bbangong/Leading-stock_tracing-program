...
def clean_stock_code(value):
    ...
    # 구글시트/CSV에서 숫자로 읽히며 5930.0 형태가 되는 경우 처리
    if code.endswith(".0"):
        code = code[:-2]
...
try:
    ...
    if df.empty:
        ...
        for _, row in df.iterrows():
            ...
            # 종목코드 괄호 포함 표시
            code_html = f'<span style="font-size: 12px; color: #B0BEC5; font-weight: 600;">({code})</span>' if code else ""
            ...
            <div style="display: flex; justify-content: space-between; align-items: center; padding: 12px 10px; margin-bottom: 10px; border-radius: 12px; background-color: #15202B; border: 1px solid #2A3644; box-shadow: 0px 4px 6px rgba(0,0,0,0.2); overflow: hidden;">
                ...
                <div style="flex: 2; text-align: left; min-width: 0;">
                    ...
                    <div style="display: flex; align-items: center; gap: 8px; margin-top: 3px; min-width: 0; font-size: 11px; white-space: nowrap; overflow: hidden;">
                        <span style="color: #FF9800; font-weight: 700; flex-shrink: 0;">
                            시총 {market_cap:,.0f}억
                        </span>
                        <span style="color: #00E676; overflow: hidden; text-overflow: ellipsis;">
                            {sector}
                        </span>
                    </div>
                </div>
